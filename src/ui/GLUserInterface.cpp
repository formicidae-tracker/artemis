...
void GLUserInterface::OnKey(int key, int scancode, int action, int mods) {
	...
	void GLUserInterface::UploadPoints(DrawBuffer & buffer) {
		...
		buffer.Points->BufferData<float, 2>(
		    GL_DYNAMIC_DRAW,
		    points.data(),
		    points.size()
		);
	...
	std::string printLine(
		...
		oss << " " << label << ":" << std::setw(size - label.size() - 1)
		    << std::setfill(' ') << std::right << std::fixed
	...
	void GLUserInterface::UploadInformations(DrawBuffer & buffer) {
		...
		dropOss << buffer.Frame.FrameDropped << " (" << std::fixed
		        << std::setprecision(2)
		        << 100.0f * (buffer.Frame.FrameDropped /
		                     float(
		...
		if (buffer.Frame.VideoOutputProcessed == -1) {
			...
			size_t totalVideo = std::max(
			    1LU,
			    buffer.Frame.VideoOutputProcessed +
			        buffer.Frame.VideoOutputDropped
			);
		...
		oss << printLine(
		    ...
		    << printLine(
		           "Tags",
		           OVERLAY_COLS,
		           buffer.Frame.Message->tags_size()
		       )
		    << std::endl
		    ...
		    << std::endl
		    << printLine("Frame Dropped", OVERLAY_COLS, dropOss.str())
		    << std::endl
