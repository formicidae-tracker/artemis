...
EuresysFrameGrabber::EuresysFrameGrabber(
    Euresys::EGenTL &gentl, const CameraOptions &options
)
	...
	if (isMaster == true) {
		...
		setInteger<DeviceModule>(
		    "StrobeDuration",
		    options.StrobeDuration.Microseconds()
		);

		DLOG(INFO) << "StrobeDelay: " << options.StrobeDelay;
		setInteger<DeviceModule>(
		    "StrobeDelay",
		    options.StrobeDelay.Microseconds()
		);
		...
		if (options.SlaveWidth == 0 || options.SlaveHeight == 0) {
			throw std::runtime_error(
			    "Camera resolution is not specified in DF mode"
			);
		}
...
EuresysFrame::EuresysFrame(
    ...
    , d_mat(
          d_height, d_width, CV_8U, getInfo<void *>(GenTL::BUFFER_INFO_BASE)
      ) {
