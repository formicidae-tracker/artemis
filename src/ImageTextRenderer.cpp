...
cv::Rect ImageTextRenderer::RenderText(
    cv::Mat           &image,
	...
	case CENTERED:
		return RenderTextAt(
		    image,
		    text,
		    cv::Point(position.x - TextWidth(text) / 2, position.y)
		);
	case RIGHT_ALIGNED:
		return RenderTextAt(
		    image,
		    text,
		    cv::Point(position.x - TextWidth(text), position.y)
		);
	default:
...
cv::Rect ImageTextRenderer::RenderTextAt(
    cv::Mat &image, const std::string &text, const cv::Point &position
) {
	Initialize();
