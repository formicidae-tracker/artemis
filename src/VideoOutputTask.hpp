...
private:
	void OverlayData(cv::Mat &frame, const Time &time, uint64_t frameID);

	void OverlayFrameNumber(cv::Mat &frame, uint64_t frameID);

	void OverlayLegacyTime(cv::Mat &frame, const Time &time);

	void OverlayTime(cv::Mat &frame, const Time &time);

	void OutputData(const std::shared_ptr<cv::Mat> &framePtr, uint64_t frameID);
