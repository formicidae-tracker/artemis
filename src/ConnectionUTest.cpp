...
public:
	...
	static void Read(const std::shared_ptr<IncommingConnection> &self) {
		boost::asio::async_read(
		    self->d_socket,
		    boost::asio::mutable_buffers_1(self->d_data, 7),
		    [self](const boost::system::error_code &ec, std::size_t) {
			    if (ec == boost::asio::error::eof) {
				    self->d_closed.SignalOne();
				    return;
			    }
			    google::protobuf::io::CodedInputStream ciss(
			        static_cast<const uint8_t *>(&(self->d_data[0])),
			        8
			    );
...
ConnectionUTest::ConnectionUTest()
	...
	d_rejectLoop = [this] {
		auto connection =
		    std::make_shared<IncommingConnection>(d_context, d_closed);
		d_rejector.async_accept(
		    connection->d_socket,
